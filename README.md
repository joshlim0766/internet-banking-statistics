...
2.1 {year, device_id(FK), rate}로 자료를 적재
2.2 {stat_master_id(FK), device_id(FK), rate}로 자료를 적재하는 형태
3. 2.2의 형태를 선택하였으며 이유는 사용율 예측 API 구현 전략을 참조
```
#### 사용율 예측 API
```
...
1.3 실제 운영 상황에서도 DB의 읽기가 80% 가량을 차지하므로 Read replica를 구성하고 Redis등으로 Cache 서버를 구성하는 구조를 채택하는 경우가 많음

...
(
    ...
    CONSTRAINT pk_device_info PRIMARY KEY (uid)
);

ALTER TABLE device_info DROP CONSTRAINT IF EXISTS uk_device_info;
ALTER TABLE device_info ADD CONSTRAINT uk_device_info UNIQUE (device_id);

CREATE INDEX IF NOT EXISTS ix_device_info ON device_info (device_name);
...
(
    ...
    CONSTRAINT fk_internet_banking_stat_detail_2 FOREIGN KEY (device_id) REFERENCES device_info(uid) ON UPDATE CASCADE ON DELETE RESTRICT
);
